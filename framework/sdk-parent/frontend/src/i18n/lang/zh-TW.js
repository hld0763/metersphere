...
const message = {
  ...
  workstation: {
    ...
    batch_sync_api_tips:
      "批量同步，有可能部分不需要同步的用例參數也被覆蓋，導致測試不通過，請謹慎操作",
    batch_ignore_case_tips:
      "注意: 批量忽略後，受影響的用例將不再有提醒，請謹慎操作",
  },
  display: {
    title: "顯示設置",
    logo: "系統 LOGO",
  ...
  organization: {
    ...
    integration: {
      ...
      zentao_prompt_information:
        "該信息為通過禪道提交缺陷的的用戶名、密碼，若未填寫，則使用工作空間中配置的默認信息",
      zentao_config_tip: "參考禪道配置文件中 $config->requestType 的值 ",
      zentao_config_path:
        "配置文件參考路徑：/opt/zbox/app/zentao/config/my.php",
      use_tip: "使用指引：",
      use_tip_tapd:
      ...
      jira_prompt_information:
        "該信息為通過Jira提交缺陷的用戶認證信息，若未填寫，則使用工作空間中配置的默認信息",
  ...
  project: {
    ...
    azureDevops_tip:
      "該信息為通過Azure Devops提交缺陷的用戶令牌信息，若未填寫，則使用工作空間中配置的默認信息",
    manager: "項目管理",
    ...
    version: {
      ...
      set_new: "設為最新版本",
      change_latest_tip:
        "此操作會修改接口，場景，測試用例等列表頁面的默認展示，可能會消耗一些時間。請耐心等待！ ",
    },
    project_file: {
      file_module_type: {
        module: "模塊",
  ...
  report: {
    ...
    project_name: "所屬項目",
    force_stop_tips:
      "<strong>強製停止</strong>測試會立刻結束當前測試並刪除報告數據",
  ...
  load_test: {
    ...
    ramp_up_tips:
      "RampUp 過大，圖表渲染卡頓不予展示，這並不影響測試執行，請以實際結果為準",
    iterate_num: "叠代次數",
    by_iteration: "按迭代次數",
    by_iteration_tip:
      "按迭代次數執行需要配置執行時長，JMeter會按照迭代次數和執行時長來決定是否結束測試",
    by_duration: "按持續時間",
  ...
  api_test: {
    ...
    definition: {
      ...
      recipient: "接收人",
      recipient_tips:
        "當API發生變化時，關聯的CASE創建人、自動化場景創建人會收到站內消息，可前往",
      select_comp: {
        no_data: "無數據",
        add_data: "去添加",
      },
    ...
    script: {
      tip_3:
        "腳本步驟會統計到場景執行結果中，執行報錯時會影響場景的最終執行結果",
  ...
  test_resource_pool: {
    ...
    batch_add_resource_tips:
      "格式：IP,Port,Monitor,最大並發數<br/>如：192.168.1.52,8082,9100,500",
  ...
  ui: {
    ...
    y: "縱坐標",
    click_tip_2:
      "默認元素的左上角為0，0；通過設置相對位置，控製鼠標在元素上的點擊位置",
    click: "單擊",
    dclick: "雙擊",
    press: "按下",
    standup: "彈起",
    ...
    select_f_tip:
      "例：比如索引值輸入 1，那麽效果會切換到當前頁面的第 2 個 frame(索引值從 0 開始計算)",
    ...
    s_w_t1:
      "例：比如索引值輸入 3，那麽效果會切換到已經打開過的第 3 個窗口(索引值從 1 開始計算)",
    ...
    wait_tip:
      "針對元素的Text屬性，指頁面展示出來的文本內容，等待超時時間為15000ms",
    exe_first: "先執行後判斷",
    ...
    instruction: "指令",
    screen_tip:
      "場景步驟如果觸發原生彈窗（alert或prompt），或不存在頁面時，截圖不生效；",
    ...
    title_tip:
      "斷言當前窗口的標題是否跟期望值一致，完全匹配則斷言成功，否則失敗",
    input_var: "請輸入變量",
    input_expect: "請輸入期望值",
  ...
  project_application: {
    workstation: {
      time_tip:
        "關閉，不設置時間範圍；開啟，根據設置的時間範圍，進入待更新列表，超出時間範圍，將自動從列表清除;",
      ...
      api_receiver_tip:
        '注意：請確認接口測試任務通知中的"API更新"已設置"站內信"消息，否則，將接收不到消息提醒',
      case_receiver_tip:
        '注意：請確認接口測試任務通知中的"CASE更新"已設置"站內信"消息，否則，將接收不到消息提醒。',
      no_show_setting_tip:
        "勾選後，不再彈出彈窗，會默認按照設置的內容執行同步和變更通知：如果需要更改設置，可以在保存按鈕旁的下拉選項中，打開設置項。",
      go_to_api_message: "接口API更新消息通知",
      go_to_case_message: "CASE更新消息通知",
      update_case_tip:
        "當CASE發生變化時，引用CASE的自動化場景創建人會收到站內消息提醒",
    },
  },
  task: {
    api_title: "接口測試任務",
